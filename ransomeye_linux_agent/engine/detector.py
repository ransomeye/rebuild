# Path and File Name : /home/ransomeye/rebuild/ransomeye_linux_agent/engine/detector.py
...
class Detector:
    ...
    def __init__(self):
        ...
        # Static rules
        self.suspicious_extensions = ['.encrypted', '.locked', '.crypto', '.vault', '.ecc', '.ezz', '.exx', '.xyz', '.aaa', '.micro', '.dharma', '.cobra']
        self.suspicious_process_names = ['encrypt', 'crypto', 'lock', 'ransom', 'wannacry', 'petya']
    ...
    def _extract_features(self, telemetry: Dict[str, Any]) -> Dict[str, float]:
        ...
        features["suspicious_paths"] = float(sum(
            1 for f in files 
            if any(path in f.get("path", "").lower() for path in self.suspicious_paths)
        ))
